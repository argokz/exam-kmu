...
3) использование словаря (dict) для хранения данных (например, соответствие «ключ → значение»);
4) работа с файлами: чтение из файла или запись в файл (open, with, read/write или readlines/writelines);
5) цикл for (можно с range());
6) проверка условий через if/elif/else;
7) обработка простого edge-case: пустой ввод / неизвестное значение / отсутствие совпадений.
...
Ограничения:
- не использовать set, def и внешние библиотеки.
